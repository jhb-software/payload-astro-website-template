...
Although uploads are stored in S3, media is **served from the Astro website at `/media/*`**, not from the CMS or the bucket directly. The `web/vercel.json` file declares a Vercel rewrite that proxies `/media/:path*` to the S3 bucket and attaches long-lived `Cache-Control` headers; the frontend's `mediaUrl` helper (`web/src/cms/mediaUrl.ts`) rewrites every CMS-supplied URL to the `/media/*` path before it is rendered.
...
- **Bucket portability** — switching providers (Hetzner → R2 → AWS S3 → …) only requires updating the rewrite destination and the `S3_*` env vars; existing image URLs in the database stay valid because they are normalized to `/media/*` at render time.

When migrating to a new bucket, update both:

1. The `S3_*` variables in `cms/.env`
2. The `destination` of the `/media/:path*` rewrite in `web/vercel.json`
